...
export default function Tasks({ data, setData }) {
  ...
  </div><div style={card}><CardHeader icon="＋" title="ADD TASK" color={C.navy2} /><div style={{ padding: 13, display: 'grid', gap: 9 }}><div><Label>TITLE</Label><Input aria-label="Task title" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Work on presentation" /></div><div><Label>DESIRED OUTCOME</Label><Input aria-label="Task desired outcome" value={form.desiredOutcome} onChange={(event) => setForm({ ...form, desiredOutcome: event.target.value })} /></div><div><Label>EXPECTED MINUTES</Label><Input aria-label="Expected minutes" type="number" min="5" max="1440" value={form.expectedMinutes} onChange={(event) => setForm({ ...form, expectedMinutes: Number(event.target.value) || 5 })} /></div><div><Label>ENERGY</Label><Select aria-label="Task energy" value={form.energy} onChange={(event) => setForm({ ...form, energy: event.target.value })} options={['low', 'medium', 'high']} /></div><div><Label>PRIORITY</Label><Select aria-label="Task priority" value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} options={['P1', 'P2', 'P3']} /></div><div><Label>DEADLINE</Label><Input aria-label="Task deadline" type="date" value={form.deadline} onChange={(event) => setForm({ ...form, deadline: event.target.value })} /></div><Button onClick={addTask} color={C.navy2} full>Add with suggested next action</Button></div></div></div>;
}
