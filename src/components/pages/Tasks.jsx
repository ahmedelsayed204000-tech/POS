...
export default function Tasks({ data, setData }) {
  ...
  const complete = (task) => {
    const now = new Date().toISOString();
    setData((previous) => ({ ...previous, tasks: (previous.tasks || []).map((item) => item.id === task.id ? { ...item, status: 'completed', updatedAt: now } : item), behaviorEvents: addEvent(previous, { id: `${now}-completed`, type: 'task_completed', occurredAt: now, entityType: 'task', entityId: task.id, metadata: {} }) }));
  };
  const postpone = (task) => {
  ...
  return <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.6fr) 300px', gap: 12, alignItems: 'start' }}><div style={{ display: 'grid', gap: 12 }}>{tasks.map((task) => <div key={task.id} style={card}><CardHeader icon={task.status === 'completed' ? '✓' : '→'} title={task.title.toUpperCase()} color={task.status === 'completed' ? C.green : C.blue} /><div style={{ padding: 13, display: 'grid', gap: 9 }}><div style={{ display: 'flex', gap: 8, fontSize: 9, color: C.g3 }}><b>{task.priority}</b><span>{task.expectedMinutes} min</span><span>{task.energy} energy</span><span>{task.status}</span>{task.postponementCount > 0 && <span>moved {task.postponementCount}×</span>}</div><div><Label>SMALLEST NEXT ACTION</Label><Input value={task.nextAction} onChange={(event) => updateTask(task.id, { nextAction: event.target.value })} /></div>{task.status === 'postponed' && <div style={{ color: C.orange, fontSize: 10 }}>{guidanceForPostponement(task.postponementReason)}</div>}<div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 6 }}><Select value={reasons[task.id] || 'unclear'} onChange={(event) => setReasons((current) => ({ ...current, [task.id]: event.target.value }))} options={postponementOptions.map(([v, l]) => ({ v, l }))} /><Button onClick={() => postpone(task)} color={C.orange} small outline>Postpone</Button><Button onClick={() => complete(task)} color={C.green} small>Complete</Button></div></div></div>)}{!tasks.length && <div style={{ ...card, padding: 24, textAlign: 'center', color: C.g3 }}>Add a task and PersonalOS will suggest an editable physical first action.</div>}</div><div style={card}><CardHeader icon="＋" title="ADD TASK" color={C.navy2} /><div style={{ padding: 13, display: 'grid', gap: 9 }}><div><Label>TITLE</Label><Input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Work on presentation" /></div><div><Label>DESIRED OUTCOME</Label><Input value={form.desiredOutcome} onChange={(event) => setForm({ ...form, desiredOutcome: event.target.value })} /></div><div><Label>EXPECTED MINUTES</Label><Input type="number" min="5" max="1440" value={form.expectedMinutes} onChange={(event) => setForm({ ...form, expectedMinutes: Number(event.target.value) || 5 })} /></div><div><Label>ENERGY</Label><Select value={form.energy} onChange={(event) => setForm({ ...form, energy: event.target.value })} options={['low', 'medium', 'high']} /></div><div><Label>PRIORITY</Label><Select value={form.priority} onChange={(event) => setForm({ ...form, priority: event.target.value })} options={['P1', 'P2', 'P3']} /></div><div><Label>DEADLINE</Label><Input type="date" value={form.deadline} onChange={(event) => setForm({ ...form, deadline: event.target.value })} /></div><Button onClick={addTask} color={C.navy2} full>Add with suggested next action</Button></div></div></div>;
}
