...
import { lifeScore, scoreFinance, scoreFitness, scoreGoals, scoreHabits, scoreLearn, scoreTime } from '../../utils/scores';
...
export default function WeeklyReview({ data, setData, dateContext }) {
  ...
  const save = () => {
    const review = { ...form, id: existing?.id || Date.now(), week: WEEK_LABEL, savedAt: new Date().toISOString(), autoScore: Number(lifeScore(data).toFixed(1)), scores: Object.fromEntries(scores.map(([name, value]) => [name, Number(value.toFixed(3))])) };
